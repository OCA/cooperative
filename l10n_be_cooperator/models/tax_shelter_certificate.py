...
def send_mail_with_additional_attachments(mail_template, res_id, attachments):
    ...
    for attachment in attachments:
        ...
        new_attachment = attachment_model.create(attachment_data)
...
class TaxShelterCertificate(models.Model):
    ...
    def _compute_access_url(self):
        ...
        for certificate in self:
            certificate.access_url = f"/my/tax_shelter_certificates/{certificate.id}"
        return result

    def generate_pdf_report(self, report_type):
        report_xmlid, name = REPORTS[report_type]
        report_action = self.env.ref(report_xmlid)
        pdf_content, _ = report_action._render_qweb_pdf(
            report_ref=report_xmlid, res_ids=self.ids
        )
        pdf_b64 = base64.b64encode(pdf_content)
        report_name = (
            f"{self.partner_id.name} {name} {self.declaration_id.name}.pdf"
        ).replace("/", "_")

        return (report_name, pdf_b64)
    ...
    def _compute_certificate_lines(self):
        for certificate in self:
            certificate.previously_subscribed_lines = certificate.lines.filtered(
                lambda r, cert=certificate: r.type == "subscribed"
                and r.transaction_date < cert.declaration_id.date_from
            )
            ...
            certificate.subscribed_lines = certificate.lines.filtered(
                lambda r, cert=certificate: r.type == "subscribed"
                and r.transaction_date >= cert.declaration_id.date_from
                and r.transaction_date <= cert.declaration_id.date_to
            )
            certificate.resold_lines = certificate.lines.filtered(
                lambda r, cert=certificate: r.type == "resold"
                and r.transaction_date >= cert.declaration_id.date_from
                and r.transaction_date <= cert.declaration_id.date_to
            )
            certificate.transfered_lines = certificate.lines.filtered(
                lambda r, cert=certificate: r.type == "transfered"
                and r.transaction_date >= cert.declaration_id.date_from
                and r.transaction_date <= cert.declaration_id.date_to
            )
