...
class WebsiteSubscription(http.Controller):
    ...
    def validation(  # noqa: C901 (method too complex)
        ...
        if kwargs.get("ordered_parts_hidden"):
            values["ordered_parts"] = int(kwargs.get("ordered_parts_hidden"))
        ...
        if error:
            ...
            _logger.warning("Missing required fields: %s", error)
            _logger.warning(
                "Values provided: %s",
                {k: v for k, v in values.items() if k not in ["error", "kwargs"]},
            )

            values = self.fill_values(values, is_company, logged)
            values.update(kwargs)
            values["error_msg"] = _(
                "Some mandatory fields have not been filled: %s" % list(error)
            )
    ...
    def share_subscription(self, **kwargs):  # noqa: C901 (method too complex)
        ...
        # Obtener ordered_parts desde ordered_parts_hidden
        if kwargs.get("ordered_parts_hidden"):
            values["ordered_parts"] = int(kwargs.get("ordered_parts_hidden"))
